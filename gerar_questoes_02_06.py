...
def gerar_md():
    ...
    # ---------------------------------------------------------
    # TEMA 1: SISTEMAS OPERACIONAIS E GESTÃO (WINDOWS SERVER)
    # ---------------------------------------------------------
    ...
    # Generate variations to reach 15 Windows questions
    for i in range(15):
        base_q = windows_questions[i % len(windows_questions)]
        content += f"#### Questão {i+1} (Sistemas Operacionais)\n\n"
        # make small modifications to ensure they look unique enough for the simulator test
        q_text = base_q['q'].replace("Um Administrador", f"O Analista {i+1}").replace("O Active Directory", f"O AD DS (cenário {i+1})")
        content += q_text + "\n\n"
        for opt in base_q['options']:
            content += opt + "\n"
        content += "\n<details><summary>Gabarito e Explicação</summary>\n\n"
        content += f"Gabarito Correto: {base_q['ans']}\n\n"
        for exp in base_q['exp']:
            content += exp + "\n"
        content += "\n</details>\n\n"
    ...
    for i in range(15, 30):
        base_q = sec_questions[i % len(sec_questions)]
        content += f"#### Questão {i+1} (Segurança da Informação)\n\n"
        q_text = base_q['q'].replace("Um firewall", f"Um equipamento de firewall (cenário {i+1})").replace("Um equipamento de segurança", f"Um ativo de segurança (cenário {i+1})")
        content += q_text + "\n\n"
        for opt in base_q['options']:
            content += opt + "\n"
        content += "\n<details><summary>Gabarito e Explicação</summary>\n\n"
    ...
    rlm_questions = [
        ...
        {
            "q": "Em uma repartição pública trabalham 120 servidores. Sabe-se que 60% deles possuem certificação ITIL, e 45% possuem certificação COBIT. Se 15% dos servidores não possuem nenhuma das duas certificações, a quantidade exata de servidores que possuem AMBAS as certificações (ITIL e COBIT) é igual a:",
            "options": [
                "A) 18.",
                "B) 30.",
                "C) 24.",
    ...
    for i in range(30, 45):
        base_q = rlm_questions[i % len(rlm_questions)]
        content += f"#### Questão {i+1} (Raciocínio Lógico)\n\n"
        q_text = base_q['q']
        if "5 programadores" in q_text:
            q_text = q_text.replace("5 programadores", f"{5 + (i%3)} programadores") # minor variations
        content += q_text + "\n\n"
        for opt in base_q['options']:
            content += opt + "\n"
        content += "\n<details><summary>Gabarito e Explicação</summary>\n\n"
